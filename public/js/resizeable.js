...
$(document).ready(function(){
	$.ajax({
		method: "GET",
		url: "https://api.upbit.com/v1/trades/ticks?market=KRW-ETH",
		success: function (d) {
			var result = d[0].trade_price.toString();;
			result = result.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
			$("#current_price span").html("1 ETH = " + result + " KRW");
		},
		fail: function (d) {
		}
	}, "json");
	...
	$("#copy_btn").click(function(){
		var obj = $("#copy_btn").parent().find("input");
		obj.select();
		document.execCommand("copy");
	})
	
	$("#copy_pub_addr_btn").click(function(){
		var obj = $("#trm_addr");
		obj.select();
		document.execCommand("copy");
	})
